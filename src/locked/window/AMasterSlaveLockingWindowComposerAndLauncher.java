...
public class AMasterSlaveLockingWindowComposerAndLauncher {
	...
	protected PeerMessageListener aMasterCoupler;
	
	
	protected ALocalModelCache cache;
	protected AMasterLockModel masterLock;
	...
	public void compose(String[] args) {
		...
		masterLock = new AMasterLockModel(communicator);
		
		addCollaborationFunctions(queue,cache,masterLock);
		doJoin();
	}
	...
	protected void addCollaborationFunctions(AnExtendibleAWTEventQueue queue, ALocalModelCache cache, AMasterLockModel masterLock) {
		addACoupler(queue);
		addASlaveCoupler(cache);
		addAMasterCoupler(masterLock);
	}
	...
	protected void addAMasterCoupler(AMasterLockModel masterLock) {
		
		aSlaveCoupler = new AMasterCoupler(masterLock);
		communicator.addPeerMessageListener(aSlaveCoupler);
	}
